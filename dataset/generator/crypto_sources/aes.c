...
void aes_mix_columns(uint8_t state[16]) {
    for (int i = 0; i < 16; i += 4) {
        ...
        uint8_t t = a ^ b ^ c ^ d;
        state[i]     ^= xtime(a ^ b) ^ t;
        state[i + 1] ^= xtime(b ^ c) ^ t;
        state[i + 2] ^= xtime(c ^ d) ^ t;
        state[i + 3] ^= xtime(d ^ a) ^ t;
    }
}

void aes_key_expansion(const uint8_t key[16], uint8_t w[176]) {
...
void aes_encrypt_block(uint8_t state[16], const uint8_t roundkeys[176]) {
